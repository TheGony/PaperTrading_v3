...
def _institutional_filter(current_price: float, closes: list, highs: list, lows: list, volumes: list):
    """VWAP 계산 + 기관 수급 필터 — 단일 역순 루프로 처리.

    TP = (High + Low + Close) / 3  (표준 전형적 가격)
    ...
    반환: (passed: bool, reason: str, vwap: float | None)
    """
    ...
    cum_tp_vol      = 0.0
    cum_vol         = 0.0
    tv_oldest_first = []  # 오래된봉→최신봉 순 거래대금 (역순 루프 결과)

    for close, high, low, vol in zip(reversed(closes), reversed(highs), reversed(lows), reversed(volumes)):
        if vol > 0:
            cum_tp_vol += (high + low + close) / 3 * vol
            cum_vol    += vol
        tv_oldest_first.append(close * vol)
...
class EntryMixin:
		...
		for attempt in range(max_retries):
			try:
				...
				for stk_cd in stocks_to_check:
					...
					prices, volumes, _, highs, lows, cntr_tm = await self._get_chart(stk_cd, needed)
					...
					if dead_cross and stk_cd in held_stock_codes:
						if rsi is not None and rsi < 45:
							...
							print(f"[{datetime.datetime.now().strftime('%H:%M:%S')}] {stk_cd}: 데드크로스 감지 but RSI {rsi_str} >= 45 - 청산 보류")

					# ── ORB 진입 분기 ──────────────────────────────────────
					if (orb_window
							and stk_cd in orb_codes
							and stk_cd not in held_stock_codes
							and stk_cd not in self.entry_time):
						await self._try_orb_entry(
							stk_cd, current_price, prices, volumes, rsi, rsi_str,
							acnt_cache=(my_stocks, aset_evlt_amt_cache),
							mkt=(kospi_flu, kosdaq_flu),
						)

					# ── MOMENTUM 진입 조건 ──────────────────────────
					if (stk_cd not in self.selected_stocks
							or stk_cd in held_stock_codes
							or stk_cd in self.entry_time):
						continue
					...
					passed, reason, vwap = _institutional_filter(
						current_price, prices, highs, lows, volumes
					)
					if not passed:
						print(
							f"[{datetime.datetime.now().strftime('%H:%M:%S')}] "
	...
	async def _try_orb_entry(self, stk_cd, current_price, _prices, volumes, rsi, rsi_str, acnt_cache=None, mkt=None):
		...
		# 갭 마이너스(-1~0%) 구간: 진입 시점에도 현재가 >= 시가 재검증
		day_open = orb.get('day_open', 0)
		orb_gap  = next((c['gap'] for c in self.orb_candidates if c['stk_cd'] == stk_cd), None)
		if orb_gap is not None and -1.0 <= orb_gap <= 0:
		...
		if cand_meta:
			...
			if price_pos < 0.90:
				log.info(f'[ORB] {stk_cd} 진입 거절: 고점 대비 위치 불량 (price_position={price_pos:.3f} < 0.90)')
				return False
