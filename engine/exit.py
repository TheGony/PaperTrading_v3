		...
		try:
			...
			for stock in my_stocks:
				if stock['stk_cd'].replace('A', '') == stk_cd:
					...
					log.info(f'[매도 시도] {stk_cd} | 수량={ord_qty} | 수익률={pl_rt:+.2f}% | reason={reason}')
					if result == 0:
						...
						log.info(f'[매도 완료] {stk_cd} | 수익률={pl_rt:+.2f}% | MFE={mfe} | MAE={mae} | 보유={held_min}분')
						completion = f"{emoji} {stock['stk_nm']} ({stk_cd}) {ord_qty}주 매도 완료\n   수익률: {pl_rt:+.2f}% | {reason}"
						tel_send(f"{signal_info}\n{completion}" if signal_info else completion)
						self._log_trade(stock['stk_nm'], stk_cd, pl_rt, reason, mfe=mfe, mae=mae, snapshot=snap, net_pl_rt=pl_rt)
					...
					if my_stocks:
						...
						for stock in my_stocks:
							...
							if not should_sell and orb_stop_pct is not None and pl_rt <= orb_stop_pct:
								should_sell = True
								hard_sell   = True
								sell_reason = f'ORB 손절 (수익률 {pl_rt:+.2f}% ≤ {orb_stop_pct:+.2f}%)'

							# 3순위: 트레일링 스탑 — 전략별 하한선
							# ORB: 0.05% (본절 사수), MOMENTUM: -0.5% (소폭 손실까지 허용)
							if not should_sell and peak >= 1.5:
								floor          = 0.05 if strategy == 'ORB' else -0.5
								actual_trigger = max(peak - trail_gap, floor)
								if pl_rt <= actual_trigger:
									should_sell = True
									floor_note  = ' [하한보존]' if actual_trigger == floor else ''
									sell_reason = (
										f'트레일링 스탑{floor_note} [{strategy}] '
										f'(고점: {peak:+.2f}% → 수익률: {pl_rt:+.2f}%, 트리거: {actual_trigger:+.2f}%)'
									)
							...
							if not should_sell and pl_rt <= stop_loss:
								should_sell = True
								hard_sell   = True
								sell_reason = f'고정 손절 [{strategy}] (수익률 {pl_rt:+.2f}% ≤ {stop_loss:+.2f}%)'
							...
							if result == 0:
								...
								self._log_trade(stock['stk_nm'], stk_cd, pl_rt, sell_reason, mfe=mfe, mae=mae, snapshot=snap_log, net_pl_rt=pl_rt)
